...
//     console.log('phones:', details.data.contacts)
//     const phones = details.data.contacts
//     console.log('phones:', phones)
//     const dummyPhoneNumber = '+918848357729'
...
const insertFall = async (req, res) => {
  try {
    ...
    // Assuming the image file is attached to the request, e.g., via a middleware like multer.
    // The image data would be in a property like 'req.file.buffer'.
    const imageFileBuffer = req.file.buffer;
    const falldetails = JSON.parse(req.body.PREDICTION_DATA);
    
    // --- STEP 1: FETCH USER DETAILS TO GET CONTACTS ---
    ...
    // --- STEP 2: DESERIALIZE CONTACTS AND SEND WHATSAPP MESSAGE ---
    // The sendMessage function now handles the image data directly.
    const contactsArray = JSON.parse(details.data.contacts);
    console.log('phones:', contactsArray);

    for (let i = 0; i < contactsArray.length; i++) {
        const phoneNumber = contactsArray[i].phoneNumber;
        const res = await sendMessage(
            imageFileBuffer, // Pass image data directly
            details.data.name,
            phoneNumber,
            falldetails.timestamp,
            falldetails.date
    ...
    // --- STEP 3: UPLOAD IMAGE AND OTHER DATA TO IPFS ---
    // This is now done after the message is sent.
    const formDataImage = new FormData();
    formDataImage.append('file', imageFileBuffer, { filename: 'fall_image.jpg' });

    const imgIPFSid = await uploadToIPFS(formDataImage, 'file');
    console.log('Image IPFS ID:', imgIPFSid);

    const dataIPFSid = await uploadToIPFS(
        falldetails,
        'json'
    );
    console.log('Data IPFS ID:', dataIPFSid);

    // --- STEP 4: STORE FALL DATA ON BLOCKCHAIN ---
    const txHash = await userOperation(
      abi.MetaSave,
      'setFallData',
      [CFAddress, imgIPFSid, dataIPFSid],
